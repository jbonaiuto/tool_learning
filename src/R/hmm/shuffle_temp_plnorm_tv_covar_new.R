...
for(s in 1:n_shuffs) {
    shuf_mat<-train
    for(trial in trials) {
        trial_idx<-which(train[,1]==trial)
	shuf_mat[trial_idx,]<-train[sample(trial_idx),]
    }
    ...
    write.csv(forward_probs,paste0(output_path, '/forward_probs_tv_plnorm_',m,'states_',run_idx,'_temp_shuf_',s,'.csv'))
}