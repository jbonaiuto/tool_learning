...
def preprocess_data(subject, recording_date, data_file_names, total_duration):
    ...
    if os.path.exists(preprocess_dir):
        ...
        for array_idx in range(len(arrays)):
            ...
            for seg in range(n_segments):
                data_file_names.append(os.path.join(preprocess_dir, 'channel_group_%d' % array_idx, 'segment_%d' % seg, 'processed_signals.raw'))

            dataio = DataIO(dirname=output_dir)
            dataio.set_data_source(type='RawData', filenames=data_file_names, dtype='float32', sample_rate=30000, total_channel=32)
