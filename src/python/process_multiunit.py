...
def run_process_multiunit(subj_name, date):
    out_dir = os.path.join(cfg['preprocessed_data_dir'], subj_name, date, 'multiunit')
    if not os.path.exists(out_dir):
        os.mkdir(out_dir)
    spike_data_dir = os.path.join(cfg['multi_unit_spike_sorting_dir'], subj_name, date, 'multiunit')
    ...
    for array_idx, region in enumerate(cfg['arrays']):
        ...
        fnames = glob(os.path.join(spike_data_dir, 'array_%d' % array_idx, '%s*spikes.csv' % region))
        for fname in fnames:
            electrode_df = pd.read_csv(fname)
            new_data={'array':[], 'electrode':[], 'trial':[], 'time':[]}
            ...
            df = pd.DataFrame(new_data, columns=['array', 'electrode', 'trial', 'time'])
...
if __name__=='__main__':
    ...
    run_process_multiunit(subject, recording_date)
