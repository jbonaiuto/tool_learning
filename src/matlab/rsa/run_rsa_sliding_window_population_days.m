function run_rsa_sliding_window_population_days(exp_info, subject, array_idx)
...
for ae_idx=1:length(align_evts)
    ...
    conv_fr=zeros(n_trials,exp_info.ch_per_array,length(data.bins)-1);
        ...
        for c_idx=1:length(conditions)    
            ...
            for t_idx=1:length(trials)
                ...
                if ~any(isinf(trial_rate))
                    conv_fr(all_t_idx,e_idx,:)=conv2(1,ones(1,200)./200,trial_rate','same');
                end
