function func_connectivity(data_dir, dates, array, electrodes, varargin)
...
granger_glm_results.array=array;
...
for date_idx=1:length(dates)
    ...
    for elec_idx=1:length(electrodes)
        ...
        if date_idx==1
            cells_to_use{elec_idx}=cells;
        else
            cells_to_use{elec_idx}=intersect(cells_to_use{elec_idx},cells);
        end
    end
end
cell_idx={};
idx=1;
for elec_idx=1:length(electrodes)
    electrode_cells_to_use=cells_to_use{elec_idx};
    electrode_cell_idx=[];
    for i=1:length(electrode_cells_to_use)
        electrode_cell_idx(i)=idx;
        idx=idx+1;
    end
    cell_idx{elec_idx}=electrode_cell_idx;
end
...
for date_idx=1:length(dates)
    ...
    for j=1:length(idx_correct)
        for elec_idx=1:length(electrodes)
            ...
            spike_data=readtable(fullfile(data_dir,date, sprintf('%s_%d_spikes.csv', array, electrode)));
            electrode_cells_to_use=cells_to_use{elec_idx};
            electrode_cell_idx=cell_idx{elec_idx};
