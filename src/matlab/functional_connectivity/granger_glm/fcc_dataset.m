...
for c = 1:length(conditionref)
    ...
    for i = 1:length(week)
        ...
        if exist(source, 'dir')
            ...
            X.Psi1.(sprintf('%s',condition{1})).(sprintf('W%d', week(i))) = temp.granger_glm_results.causal_results.Psi1;
            X.Psi2.(sprintf('%s',condition{1})).(sprintf('W%d', week(i))) = temp.granger_glm_results.causal_results.Psi2;
            X.Phi.(sprintf('%s',condition{1})).(sprintf('W%d', week(i))) = temp.granger_glm_results.causal_results.Phi;
