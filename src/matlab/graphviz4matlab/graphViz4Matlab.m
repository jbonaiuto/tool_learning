        ...
        function displayEdges(obj,indices)
            ...
            for i=1:numel(indices)
                ...
                if edge.from~=edge.to
                    ...
                    RArc=edge.from.width/2;
                    R=edge.from.width/2;
                    %teta=pi;
                    teta=0;
                    ArrowSize=obj.maxNodeSize/8;
%                     SelfArcArrow(X(k),Y(k),...
%                         Teta,ArrowSize,ArcR,R,...
%                         MyGraph.EdgeLabels{k,k},'Color', RGB);
                    %ArcR=ArcR+ArrowSize;

                    Angles=obj.SolveTriangle([RArc RArc R]);
                    ArcStart=Angles(3)-pi/2;
                    ArcEnd=pi+(pi/2-Angles(3));
                    ArcT=linspace(ArcStart,ArcEnd,30);
                    ArrowAngles=obj.SolveTriangle([RArc RArc ArrowSize]);
                    ...
                    X2=Rot(1,:)+edge.from.xpos;
                    Y2=Rot(2,:)+edge.from.ypos;
                    ArrowX2=RotArrow(1,:)+edge.from.xpos;
                    ArrowY2=RotArrow(2,:)+edge.from.ypos;
                    edge.arrow=plot(X2,Y2,'LineStyle',obj.edgeLineStyle,'LineWidth',2,'HitTest','off','Color',edgeColor);                    
                    if(~obj.undirected)
                        %arrowHead = obj.displayArrowHead(X2,Y2,ArrowX2,ArrowY2,edgeColor);
                        arrowHead=obj.PlotArrow(ArrowX2,ArrowY2,ArrowSize,0,edgeColor);
                        edge.arrow = [edge.arrow arrowHead];
                    end
                end
                edgeLabel = ''; 
...
%                 edge.label = text(mean(X)+0.02*normal(1),mean(Y)+0.02*normal(2),edgeLabel,'FontSize',round(obj.fontSize*2/3),'HorizontalAlignment','center','Rotation',-90+atan2(normal(2),normal(1))*180/pi);
        ...
        function arrowHead=PlotArrow(obj, X,Y,Size,Trim,arrowColor,varargin)
            P1=[X(1) Y(1)];
            P2=[X(2) Y(2)];
            P=P2-P1;
            OL=norm(P);
            ...
            ArrX=([0 2 1]-1)*Size/H;
            ArrY=([0 0 H]-H)*Size/H;
            %arrowHead=plot(X,Y,'Color',arrowColor,varargin{:});
            RotXY=A*[ArrX;ArrY];
            arrowHead=fill(X(2)+RotXY(1,:),Y(2)+RotXY(2,:),arrowColor);
            set(arrowHead,'EdgeColor',arrowColor);
        end
    ...
    methods
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function [Angles]=SolveTriangle(obj,Edges)
