function plot_visual_rake_push_movement_times(exp_info, subject)

CIFcn = @(x,p)prctile(x,abs([0,100]-(100-p)/2));

conditions={'visual_rake_push_left','visual_rake_push_right'};
...
med_tt=median(all_tts)
ci_tt=CIFcn(all_tts,95)
...
xlabel('RT');
title('Visual rake push');
subplot(4,1,2);
hist(all_tts,100);
hold all;
plot([med_tt med_tt],ylim(),'k--');
plot([ci_tt(1) ci_tt(1)],ylim(),'r--');
plot([ci_tt(2) ci_tt(2)],ylim(),'r--');
xlabel('Tool grasp');
subplot(4,1,3);
hist(all_mts,100);
hold all;
