function f=plot_multialign_multiunit_array_data(exp_info, subject, dates, array, conditions)
...
if length(condition_trials)>2
  ...
  for r=1:length(align_events)
      ...
      if r==1
	  ylabel({sprintf('%s, %s', strjoin(conditions,', '), array);' ';'Firing rate'});
      end
      handles=[];
      electrode_labels={};
