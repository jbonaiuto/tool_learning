function [aligned_forward_probs,f]=plotHMM_aligned_condition_onetrial(data, dates,...
...
for cond_idx=1:length(conditions)
    ...
    condition_trials = find(strcmp(data.metadata.condition,conditions));
    ...
    for r=1
        ...
        for d=8 %the number of the data I'm working on in the data structure        1:length(dates)
            ...
            for n=1:length(day_trials)
                ...
                trial_firing_rates=squeeze(data.smoothed_firing_rate(1,:,day_trials(n),:));
...
for t_idx=1:length(day_trials)
    ...
    subplot(2,1,1);
    electrode_labels={};
    for e=1:length(data.electrodes)
        %plot(data.bins(bin_idx)-event_time,trial_firing_rates(:,bin_idx)');
        plot([win_size(1):binwidth:win_size(2)],squeeze(cond_firing_rates(t_idx,e,r,:)));
        hold on
        electrode_labels{end+1}=sprintf('electrode %d',e);
    end
