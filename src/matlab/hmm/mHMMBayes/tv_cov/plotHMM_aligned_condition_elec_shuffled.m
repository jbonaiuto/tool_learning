function [aligned_forward_probs,f]=plotHMM_aligned_condition_elec_shuffled(subject, array, data, dates,...
...
for cond_idx=1:length(conditions)
    ...
    for shuf_idx=1:params.n_shuffs
        ...
        cond_forward_probs(shuf_idx,:,:,:)=mean(shuf_forward_probs);
    end
...
saveas(f,fullfile(output_path,...
     [subject '_' array '_' 'grasp' '_StateSequence_Elecshuff_10d_TvCov_StdDev' '.png']));
saveas(f,fullfile(output_path,...
     [subject '_' array '_' 'grasp' '_StateSequence_Elecshuff_10d_TvCov_StdDev' '.eps']),'epsc');
end