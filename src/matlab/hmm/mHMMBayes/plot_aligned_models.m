...
for d_idx=1:length(dates)/2
    ...
    %plotHMM_aligned_condition(data, dates(d_idx), conditions, model);
    %plot_fwd_probs_event_sorted(data, model, conditions(cond_idx), dates)
...
for d_idx=1:length(singleday_models)
    ...
    plotHMM_aligned_condition(datasets{d_idx}, dates((d_idx-1)*2+1:d_idx*2), conditions, singleday_models(d_idx));
