function aligned_p_states=plotHMM_model_aligned(data, date, conditions, array, model, metric, variable)
...
for i=1:length(conditions)
    ...
    cond_rows=find(model.forward_probs.condition==i);    
    cond_trials=unique(model.forward_probs.subj(cond_rows));
    ...
    for r=1:length(align_events)
        ...
        for g = 1:length(trials)
            ...
            if ~isnan(data.metadata.place(trials(g)))
                trial_rows=find(model.forward_probs.subj==cond_trials(t_idx));
...
% date=strrep(date,'.','-');
% 
% saveas(f,fullfile(exp_info.base_output_dir,'HMM', 'betta', 'grasp','50_models', [date '_' array '_condFR_grasp.png']));
% saveas(f,fullfile(exp_info.base_output_dir,'HMM', 'betta', 'grasp','50_models', [date '_' array '_condFR_grasp.eps']), 'epsc');
% close(f);
...
saveas(f,fullfile(exp_info.base_output_dir,'HMM', 'betta', 'grasp','50_models','model_aligned', [date '_' array '_' metric '_' variable '_aligned_grasp.png']));
saveas(f,fullfile(exp_info.base_output_dir,'HMM', 'betta', 'grasp','50_models','model_aligned', [date '_' array '_' metric '_' variable '_aligned_grasp.eps']), 'epsc');
close(f);
end