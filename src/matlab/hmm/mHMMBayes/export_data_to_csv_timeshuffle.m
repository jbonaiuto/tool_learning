function concat_data=export_data_to_csv_timeshuffle(exp_info, subject, array,...
...
for shuff_idx=1:shuffle_nbr
    ...
    data_file=fullfile(output_path, sprintf('hmm_data_timeshuffle_%d.csv',shuff_idx));
    ...
    for trial_idx = 1:length(condition_trials)
        ...
        trial_spikes=squeeze(concat_data.binned_spikes(1,:,trial_idx,shuff_bin_idx));
        ...
        for electrode_idx=1:length(good_electrodes)
            for time_idx=1:size(trial_spikes,2)
                time_spikes=trial_spikes(good_electrodes(electrode_idx),time_idx);
                line=sprintf('%d,%d,%d,%d,%d,%d',...
                    trial_date_idx,...
                    trial_idx,...
                    condition_idx,...
                    electrode_idx,...
                    shuff_bin_idx(time_idx),...
                    time_spikes);
                fprintf(fid,'%s\n',line);
            end
        end
