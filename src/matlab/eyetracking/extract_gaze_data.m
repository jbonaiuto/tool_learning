function extract_gaze_data(exp_info, subject, date)
...
load(fullfile(out_path, sprintf('%s_%s_calibration.mat', subject, date)));

plex_files=unique(info.plexon_file);
plex_trial_data={};
data_path=fullfile(exp_info.base_data_dir, 'recordings/plexon', subject, date);

for plx_idx=1:length(plex_files)
    if length(plex_files{plx_idx})>0
        % Load plexon file
        plex_data=readPLXFileC(fullfile(data_path, plex_files{plx_idx}),'all');
